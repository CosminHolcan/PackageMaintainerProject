...
public class MaintainerServiceImpl implements MaintainerService{
    ...
    public void deleteMaintainer(Long ID){
        ...
        var maintainer = this.maintainerRepo.findById(ID).orElseThrow(() -> new AppException("Non-existent maintainer with this ID !"));
        maintainer.prepareToBeDeleted();
    ...
    public List<Maintainer> getAllMaintainers() {
        ...
        var maintainers = this.maintainerRepo.findAllWithPackageMaintainers();
        log.trace("allMaintainers - result={}", maintainers);
        return new ArrayList<>(maintainers);
    }

    @Override
    public Maintainer getOneMaintainer(Long ID) {
        return this.maintainerRepo.findOneWithPackageMaintainersJPQL(ID).orElseThrow(() -> new AppException("Non-existent maintainer !"));
    //    return this.maintainerRepo.findOneWithPackageMaintainersCriteria(ID).orElseThrow(() -> new AppException("Non-existent maintainer !"));
    }
}
