...
public class PackageServiceImpl implements PackageService{
    ...
    public void deletePackage(Long ID){
        ...
        var aPackage = this.packageRepo.findById(ID).orElseThrow(() -> new AppException("Non-existent package with this ID !"));
        aPackage.prepareToBeDeleted();
        var packageVersions = this.packageVersionRepository.findAllByAPackage(aPackage);
        packageVersions.stream()
                .forEach(p ->this.packageVersionRepository.deleteById(p.getId()));
    ...
    public List<Package> getAllPackages() {
        ...
        var packages = this.packageRepo.findAllWithPackageMaintainers();
    ...
    public Package getOnePackage(Long ID) {
        return this.packageRepo.findOneWithPackageMaintainersCriteria(ID).orElseThrow(() -> new AppException("Non-existent package with this ID !"));
    //    return this.packageRepo.findOneWithPackageMaintainersJPQL(ID).orElseThrow(() -> new AppException("Non-existent package with this ID !"));

    }

    @Override
    public Package getPackageByName(String name) {
        return this.packageRepo.findOneByNameWithPackageMaintainersJPQL(name).orElseThrow(() -> new AppException("Non-existent package with this name !"));
    }
}
